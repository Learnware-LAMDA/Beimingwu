...
def init_engine():
    ...
    if config.engine["type"] == "easy":
        engine = instantiate_learnware_market(market_id="backend_hetero", name="eazy", rebuild=False)
        engine_config = leanrware_conf
    elif config.engine["type"] == "hetero":
        engine = instantiate_learnware_market(
            market_id="backend_hetero", name="hetero", rebuild=False, organizer_kwargs={"auto_update": False}
        )
        engine_config = leanrware_conf
    else:
